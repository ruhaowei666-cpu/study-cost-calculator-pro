...
def main():
    ...
    if not is_logged_in():
        ...
        st.markdown("---")
        st.info("💡 注册账号后即可使用所有功能。免费用户每月可计算3次，付费用户无限制。")
        return
    ...
    with col_title:
        st.markdown('<div class="main-header">💰 留学生成本计算器</div>', unsafe_allow_html=True)
    with col_user:
        with st.popover(f"👤 {user_email.split('@')[0]}"):
            st.write(f"**邮箱**: {user_email}")
            st.write(f"**订阅**: {'专业版' if usage_info['is_pro'] else '免费版'}")
            if not usage_info['is_pro']:
                st.write(f"**本月使用**: {usage_info['monthly_usage']}/{usage_info['monthly_limit']} 次")
            if usage_info.get('expires_at'):
                st.write(f"**到期时间**: {usage_info['expires_at'].strftime('%Y-%m-%d')}")
            st.markdown("---")
            if st.button("📊 查看历史记录", use_container_width=True):
                st.session_state['show_history'] = True
            if st.button("📈 使用统计", use_container_width=True):
                st.session_state['show_stats'] = True
            if st.button("💳 订阅管理", use_container_width=True):
    ...
    if not usage_info['is_pro']:
        ...
        if remaining and remaining > 0:
            st.info(f"💡 {usage_info['remaining']}/{usage_info['monthly_limit']} 次免费计算剩余")
        elif remaining == 0:
            st.warning(f"⚠️ 您本月已用完免费计算次数。请升级到专业版享受无限计算。")
    ...
    if calculate_button:
        ...
        if not can_calculate:
            ...
            st.info("💡 请升级到专业版享受无限计算，或等待下月重置免费次数。")
...
def show_subscription_page(user_id: int, subscription_manager: SubscriptionManager):
    ...
    col1, col2 = st.columns(2)
    with col1:
        st.metric("当前订阅", "专业版" if usage_info['is_pro'] else "免费版")
    with col2:
        if usage_info['is_pro']:
            if usage_info.get('expires_at'):
                st.metric("到期时间", usage_info['expires_at'].strftime('%Y-%m-%d'))
        else:
            st.metric("本月剩余", f"{usage_info['remaining']}/{usage_info['monthly_limit']} 次")
    ...
    if not usage_info['is_pro']:
        ...
        st.success("✅ 您已经是专业版用户，享受所有功能！")
        if usage_info.get('expires_at'):
            days_left = (usage_info['expires_at'] - datetime.now()).days
            if days_left <= 7:
                st.warning(f"⚠️ 您的订阅将在 {days_left} 天后到期，请及时续费。")
...
def show_history_page(user_id: int, db: Database):
    ...
    if not calculations:
        st.info("📝 您还没有计算记录。完成第一次计算后，记录会显示在这里。")
        if st.button("← 返回", use_container_width=True):
            st.session_state['show_history'] = False
            st.rerun()
        return
    
    st.write(f"共找到 {len(calculations)} 条记录")
    
    # 显示历史记录列表
    for i, calc in enumerate(calculations):
        with st.expander(f"📅 {calc['created_at'][:19]} - {calc['country']} {calc['city']}"):
            col1, col2 = st.columns(2)
            
            with col1:
                st.write("**输入信息**")
                inputs = calc['inputs']
                st.write(f"- 国家/城市: {inputs.get('country')} / {inputs.get('city')}")
                st.write(f"- 住宿: {inputs.get('rent_type')}")
                st.write(f"- 打工: {'是' if inputs.get('has_job') else '否'}")
                if inputs.get('has_job'):
                    st.write(f"- 小时工资: {inputs.get('hourly_wage')}")
                st.write(f"- 初始存款: {inputs.get('initial_deposit')}")
                st.write(f"- 学费: {inputs.get('tuition_total')}")
            
            with col2:
                st.write("**计算结果**")
                results = calc['results']
                st.write(f"- 月收入: {results.get('monthly_income', 0):.2f}")
                st.write(f"- 最终余额: {results.get('final_balance', 0):.2f}")
                st.write(f"- 最低余额: {results.get('min_balance', 0):.2f}")
                if results.get('critical_months'):
                    st.write(f"- 危险月份: {', '.join(results['critical_months'])}")
            
            col_del, col_view = st.columns(2)
            with col_del:
                if st.button("🗑️ 删除", key=f"delete_{calc['id']}"):
                    if db.delete_calculation(user_id, calc['id']):
                        st.success("已删除")
                        st.rerun()
            with col_view:
                if st.button("👁️ 查看详情", key=f"view_{calc['id']}"):
